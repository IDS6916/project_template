...
>>H2 Rationale: A major challenge for the current projection will be attempts to validate our model against historical data. Currently, there are some documented drug smuggling routes that have been made public throughout Central and North America that will be used as validation data. However, drug smuggling routes may differ from artificat smuggling routes, and that drug smuggling data is limited to begin with. Administering a survey where participants are asked to make various route decision given the historical context, we hope to gain more insight about smugglign behavior and provide further validation for our predicted routes.

H3: Geopolitical conflict will influence region-specific routes. 

>> H3 Rationale: One assumption that we are operating under is that smuggler will generally behave like ordinary citizen in order to lower their visibility to law enforcement. One concern is that--in countries with political instability--law enforcement may not serve as a destriment to smugglers if police corruption is an issue. To better understand how regional instability may influence smuggling and law enforcement behavior, we plan to distribute a qualitative survey to participants who live(d) in those regions.
